...
def train_type2_model(disc_model, cup_model, dataset_path, cup_masks_path, device):
    ...
    # Unfreeze and modify the last two layers
    cup_model.dec1 = UNet.conv_block(128 + 64, 32).to(device)
    cup_model.final = nn.Conv2d(32, 2, 1).to(device)

    for param in cup_model.dec1.parameters():
        param.requires_grad = True
    for param in cup_model.final.parameters():
        param.requires_grad = True
...
def main():
    ...
    cup_model_type2.load_state_dict(disc_model.state_dict(), strict=False)
    cup_model_type2 = train_type2_model(disc_model, cup_model_type2, CONFIG['dataset_path'], CONFIG['cup_masks_path'], device)
    # Training Cup Segmentation Model (Type 2)
    # logging.info("\nTraining Cup Segmentation Model (Type 2)")
    # cup_model_type2 = UNet(n_classes=2, in_channels=3).to(device)
    # cup_model_type2.load_state_dict(disc_model.state_dict())
    # cup_model_type2.final = nn.Conv2d(64, 2, 1)  # Modify the final layer for binary segmentation
    # cup_model_type2 = train_type2_model(disc_model, cup_model_type2, CONFIG['dataset_path'], CONFIG['cup_masks_path'], device)
