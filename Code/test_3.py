...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, idx):
        try:
            ...
            print(f"Error loading image or mask at index {idx}: {str(e)}")
            return None
...
#     return accuracy, jaccard, dice
    ...
    #     dice_scores = []
    #     for p, t in zip(all_preds, all_targets):
    #         dice_scores.append(dice_coefficient(torch.tensor(p), torch.tensor(t)).item())
...
def dice_coefficient(pred, target):
    ...
    # If pred is a scalar, convert it to a 1D tensor
    if pred.dim() == 0:
        pred = pred.unsqueeze(0)
    
    # If pred is 1D, treat it as already binary
    if pred.dim() == 1:
        pred = (pred > 0.5).float()
    else:
        # For 2D or higher, apply softmax and threshold
        if pred.size(1) > 1:
...
def train_type1_model2(disc_model, dataset_path, cup_masks_path):
    ...
    start_time = time.time()
    train_losses, val_losses, train_dice_scores, val_dice_scores = train_model(
        model, train_loader, val_loader, criterion, optimizer, scheduler, device, CONFIG['num_epochs'], 'best_cup_model.pth'
    )
    total_time = time.time() - start_time
    print(f"Total training time: {total_time:.2f} seconds")
...
def train_type2_model(disc_model, dataset_path, cup_masks_path):
    ...
    start_time = time.time()
    train_losses, val_losses, train_dice_scores, val_dice_scores = train_model(
        cup_model, train_loader, val_loader, criterion, optimizer, scheduler, device, CONFIG['num_epochs'], 'best_cup_model_type2.pth'
    )
    total_time = time.time() - start_time
    print(f"Total training time: {total_time:.2f} seconds")
